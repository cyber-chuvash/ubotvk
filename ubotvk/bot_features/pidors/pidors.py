...
class Database:
    ...
    def create_if_not_exists(self):
        ...
        cursor = conn.cursor()
        cursor.execute("""CREATE TABLE IF NOT EXISTS Pidors (chat_id, user_id, user_name, user_pidor_count)""")
    ...
    def add_member(self, chat_id, member):
        ...
        conn = sqlite3.connect(self.db_file)
        cursor = conn.cursor()
        cursor.execute("""INSERT INTO Pidors (chat_id, user_id, user_name, user_pidor_count) VALUES (?, ?, ?, ?)""",
    ...
    def get_pidors(self, chat_id):
        conn = sqlite3.connect(self.db_file)
        cursor = conn.cursor()
        cursor.execute("""SELECT * FROM Pidors WHERE Pidors.chat_id=?""", (chat_id,))
    ...
    def add_chat(self, chat_id):
        conn = sqlite3.connect(self.db_file)
        cursor = conn.cursor()
        cursor.execute("""INSERT INTO Chats (chat_id, feature_is_on) VALUES (?, ?)""", (chat_id, 1))    # TODO check if chat_id already in db
        conn.commit()
        conn.close()
